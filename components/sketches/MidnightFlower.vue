    ...
    mounted() {
        let sketch = function (p) {
            ...
            p.setup = function () {
                ...
                radiusSlider = createSliderWithLabel(
                    0,
                    250,
                    default_radius,
                    1,
                ...
                verticesSlider = createSliderWithLabel(
                    0,
                    15,
                    default_vertices,
                    1,
                ...
                periodSlider = createSliderWithLabel(
                    0,
                    10,
                    default_period,
                    0.5,
                ...
                widthSlider = createSliderWithLabel(
                    0,
                    250,
                    default_width,
                    1,
                ...
                layersSlider = createSliderWithLabel(
                    0,
                    25,
                    default_layers,
                    1,
                ...
                lineCheckbox = p
                    .createCheckbox("Petal Lines", default_linesCheck)
                    .position(sliderX, sliderY);
                rotateCheckbox = p
                    .createCheckbox("Rotate", default_rotateCheck)
                    .position(sliderX + 100, sliderY);
                ...
                function createSliderWithLabel(
                    ...
                    let slider = p
                        .createSlider(min, max, starting, step)
                        .position(sliderX, sliderY)
                        .style("width", "150px");

                    p.createDiv(label) /*.style('background-color', '#EBEBEB')*/
            ...
            p.draw = function () {
                ...
                for (let i = 0; i < vertices * layers; i++) {
                    ...
                    let end = p5.Vector.fromAngle(
                        p.radians(
                            i * interAngle +
                                ((p.floor(i / vertices) % period) *
                                    interAngle) /
                                    period
                        )
                    );
                    ...
                    let brightnessOffset =
                        p.brightness(petalColor) / (layers * 2) + 1;
                    //petalColor = p.color(p.hue(petalColor), p.saturation(petalColor), p.brightness(petalColor) - layer*7);
                    petalColor = p.color(
                        p.hue(petalColor),
                        p.saturation(petalColor),
                        p.brightness(petalColor) - brightnessOffset * layer
                    );
                    outlineColor = p.color(
                        p.hue(outlineColor),
                        p.saturation(outlineColor),
                        p.brightness(outlineColor)
                    );
            ...
            function drawPetal(
                ...
                let widthHypotenuse = p.sqrt(
                    thirdLength * thirdLength + width * width
                );
                ...
                let control1 = p5.Vector.fromAngle(
                    angleFromZero - widthAngle
                ).setMag(widthHypotenuse);

                // this point will be along the central axis
                // and 75% of the full length
                // in summary, controls the "notch" on the end
                let control2 = p5.Vector.fromAngle(angleFromZero).setMag(
                    length * 0.75
                );
